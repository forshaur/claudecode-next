"""Claude.ai streaming – reverse‑engineered REST/SSE client."""
...
def _feed_sse_line(line, parts, quiet=False):
    """Parse one SSE 'data: ...' line, appending text to parts.
...
def _handle_rate_limit(resp, creds, prompt, model, discrete, quiet=False, system_prompt=None):
    """Handle 429: show wait time, auto‑fallback to haiku."""
    retry = ""
    try:
...
def stream_prompt(creds, prompt, model=DEFAULT_MODEL, discrete=True,
                  session_state=None, quiet=False, system_prompt=None):
    ...
    If discrete=True, that conversation is deleted when the session ends.
    session_state dict tracks: {'conv_id': str, 'created': bool}.
    If quiet=True, no output to stdout. If system_prompt given, prepended.
    """
...
# For backward compatibility – keep the old http module name if needed
# (but we'll not import this directly from now on)